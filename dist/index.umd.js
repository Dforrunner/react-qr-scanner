(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('react'), require('jsqr')) :
  typeof define === 'function' && define.amd ? define(['exports', 'react', 'jsqr'], factory) :
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.YourLibraryName = {}, global.React, global.jsQr));
})(this, (function (exports, React, jsQR) { 'use strict';
  ...
  const decoder = imageData => new Promise(resolve => {
    const decoded = jsQR(imageData.data, imageData.width, imageData.height);
    if (decoded) resolve(decoded.data);else resolve();
  });
  const QrScanner = ({
    onError = error => console.log({
      error
    }),
    onScan = value => console.log({
      value
    }),
    ...
    const initiate = () => {
      ...
      let supported = {};
      if (navigator.mediaDevices && typeof navigator.mediaDevices.getSupportedConstraints === 'function') supported = navigator.mediaDevices.getSupportedConstraints();
      const constraintsDef = {};
      if (supported.facingMode) {
        constraintsDef.facingMode = {
          ideal: facingMode
        };
      }
      if (supported.frameRate) {
        constraintsDef.frameRate = {
          ideal: 25,
          min: 10
        };
      }
      const vConstraintsPromise = supported.facingMode || isFirefox ? Promise.resolve(constraints || constraintsDef) : getDeviceId(facingMode).then(deviceId => Object.assign({}, {
        deviceId
      }, constraints));
      vConstraintsPromise.then(video => navigator.mediaDevices.getUserMedia({
        video
    ...
    }, showViewFinder ? /*#__PURE__*/React.createElement("div", {
      style: viewFinderStyle
    }) : null, /*#__PURE__*/React.createElement("video", {
      style: videoPreviewStyle,
      ref: setRefFactory('preview')
    }), /*#__PURE__*/React.createElement("canvas", {
      style: hiddenStyle,
      ref: setRefFactory('canvas')
    })));
  };
